...
the enabled translation mechanisms.
...
modified by the `MPRV`, `MPV`, and `MPP` bits in `mstatus` and the `SUM` bit in
either `mstatus/sstatus` or `vsstatus`. The `MXR` bit in either
...
following standard transformation is defined for cache-block maintenance
...
Executing a cache-block management instruction or a cache-block zero instruction
may cause a reservation on another hart to be lost. As a result, the following
is added to the list of events that satisfy the eventuality guarantee of
constrained LR/SC loops, as defined in the A extension:
...
_Executing a cache-block prefetch instruction does not impact the eventuality
guarantee of any hart's constrained LR/SC loop, as defined in the A extension._
