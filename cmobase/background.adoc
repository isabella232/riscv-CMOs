...
When a CMO instruction traps, zero is written into the appropriate trap
instruction CSR, `mtinst` or `htinst`, in accordance with the hypervisor
extension.

****

_The existing store transformation is unsuitable since the upper 12b of the CMO
instruction encoding is used for opcode information rather than immediate and
rs2._
...
* Define misaligned exception for CBO.ZERO to emulate operations for 
